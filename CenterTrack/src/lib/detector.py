...
class Detector(object):
  ...
  def run(self, image_or_path_or_tensor, meta={}):
    ...
    if isinstance(image_or_path_or_tensor, np.ndarray):
      ...
      self.image = image_or_path_or_tensor['image'][0].numpy()
    ...
    for scale in self.opt.test_scales:
      ...
      if self.opt.tracking:
        ...
        if self.pre_images is None:
          ...
          self.color_dict = dict()
          self.pre_tracks = []
          self.pre_image = copy.deepcopy(self.image)
        if self.opt.pre_hm:
    ...
    pre_results = self.tracker.tracks
    ...
    imgs_save_dir = os.path.join(self.opt.save_dir, 'imgs_results_mot17_new')
    if not os.path.exists(imgs_save_dir):
      os.mkdir(imgs_save_dir)
    num_pre = 0
    num_now = 0
    position_pre_dict = dict()
    ...
    if len(pre_results):
      for item in pre_results:
        ...
        if not (tracking_id in self.color_dict.keys()):
          r = random.randint(0, 255)
          g = random.randint(0, 255)
          b = random.randint(0, 255)
          self.color_dict[tracking_id] = (b, g, r)
        ...
        if not (tracking_id in position_pre_dict.keys()):
          position_pre_dict[tracking_id] = [(bbox[0]+bbox[2])/2 , (bbox[1]+bbox[3])/2]

        cv2.rectangle(pre_image, (x1, y1), (x2, y2), self.color_dict[tracking_id], 2)
        cv2.putText(pre_image, 'Id: %s' % tracking_id, (x2 + 10, y1), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                    self.color_dict[tracking_id], 2)
        cv2.putText(pre_image,
                    'p: (%.1f,' % position_pre_dict[tracking_id][0] + '%.1f)' % position_pre_dict[tracking_id][1],
                    (x2 + 10, y1 + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, self.color_dict[tracking_id], 2)
    cv2.putText(pre_image, 'number: %s' % num_pre, (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255), 2)
    self.pre_image = image
    #cv2.imshow('visualization', pre_image)
    #cv2.waitKey(0)
    ...
    for item in results:
      ...
      position_now = [(bbox[0]+bbox[2])/2 , (bbox[1]+bbox[3])/2]

      cv2.rectangle(image, (x1, y1), (x2, y2), self.color_dict[tracking_id], 2)
      cv2.putText(image, 'Id: %s' % tracking_id, (x2 + 10, y1), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                  self.color_dict[tracking_id], 2)
      cv2.putText(image, 'p: (%.1f,' % position_now[0] + '%.1f)' % position_now[1], (x2 + 10, y1 + 20),
                  cv2.FONT_HERSHEY_SIMPLEX, 0.6, self.color_dict[tracking_id], 2)
      if tracking_id in position_pre_dict:
        v = math.sqrt((position_now[0] - position_pre_dict[tracking_id][0]) ** 2 + (
                position_now[1] - position_pre_dict[tracking_id][1]) ** 2)
        cv2.putText(image, 'v: %.2f' % v, (x2 + 10, y1 + 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, self.color_dict[tracking_id],
                    2)

    cv2.putText(image, 'number: %s' % num_now, (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255), 2)
    imgs = np.hstack([pre_image, image])
    cv2.namedWindow('visualization', cv2.WINDOW_FREERATIO)
    cv2.setWindowProperty('visualization', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
    cv2.imshow('visualization', imgs)
    keyvalue = cv2.waitKey(1)
    if keyvalue & 0xFF == ord(' '):  # 按空格键时，暂停在当前帧
      cv2.waitKey(0)
    if keyvalue == 27:  # 按esc键时，退出程序
      exit(0)
    img_name = os.path.join(imgs_save_dir, '%06d.jpg' % (self.cnt))

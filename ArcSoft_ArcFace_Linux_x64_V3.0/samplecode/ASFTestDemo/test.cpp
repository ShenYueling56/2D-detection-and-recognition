//
// Created by xuzz27 on 2021/4/13.
//
...
int main()
{
    ...
    //char* picPath_test = "/face/ArcSoft_ArcFace_Linux_x64_V3.0/samplecode/ASFTestDemo/images/biden.jpg";
    printf("failed");
    //Mat src = imread(picPath_test );
    IplImage* originalImg = cvLoadImage("../images/biden.jpg");
    ...
    if (img)
    {
        MRESULT res = ASFDetectFacesEx(handle, &offscreen, &detectedFaces4);
        ...
        {
            // 打印人脸检测结果
            for (int i = 0; i < detectedFaces4.faceNum; i++)
            {printf("Face Id: %d\n", detectedFaces4.faceID[i]);
                printf("Face Orient: %d\n", detectedFaces4.faceOrient[i]);
                printf("Face Rect: (%d %d %d %d)\n",
                       detectedFaces4.faceRect[i].left, detectedFaces4.faceRect[i].top,
                       detectedFaces4.faceRect[i].right,
                       detectedFaces4.faceRect[i].bottom);
            }
    ...
    if (fp1 && fp2 && fp3)
    {
        ...
        fclose(fp3);
        //fread(imageData4, 1, Height*Width, fp4);		//读NV21前2/3的数据,用于红外活体检测
        //fclose(fp4);
